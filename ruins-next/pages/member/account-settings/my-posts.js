...
import { API_SERVER, TOUR_GET_POST, TOUR_DELETE_POST, TOUR_EDIT_POST } from '@/components/config/api-path'
              ...
              <div className="">
                {posts.map((post, i) => (
                  <div id="articleCard" className="w-full pb-10" key={i}>
                    ...
                    >
                      <img
                        src={post.image_url && post.image_url.startsWith('/img') ? `${API_SERVER}${post.image_url}` : `/images/borou/${post.image_url}.jpg`}
                      ...
                      <div className="relative space-y-2.5 w-full">
                        ...
                        <button
                          className="md:absolute bottom-0 right-[230px] md:w-48 w-1/2 md:py-4 py-2 bg-white text-black hover:bg-red-900 hover:text-white hover:border-white italic"
                          onClick={(e) => handleDeletePost(post.tour_id, e)}
                        >
                          DELETE
                        </button>
                        <button
                          className="md:absolute bottom-0 right-0 md:w-48 w-1/2 md:py-4 py-2 bg-black hover:bg-white hover:text-black italic"
