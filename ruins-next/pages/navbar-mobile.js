...
export default function NavbarMobile() {
  ...
  const handleArrowLR = () => {
    ...
    setSelectedDropdown(null) // Reset selected dropdown
  }
  ...
  return (
    <>
      <div className="w-[375px] bg-zinc-800 relative">
        ...
        <div
          id="expand"
          className={`px-[30px] absolute top-full left-0 w-full ${isLeftIcon && selectedDropdown ? 'block' : 'hidden'}`}
        >
          {isLeftIcon &&
            selectedDropdown && ( // Only show expand if left icon is clicked and a dropdown is selected
